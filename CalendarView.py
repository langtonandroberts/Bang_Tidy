...
class CalendarView:
	...
	def setup(self, y, m):
		...
		for num, name in enumerate(days):
			t = Label(self.parent, text=name[:3],bg=self.COLOR_OF_CALENDAR_LABEL)
			self.wid.append(t)
		...
		print("You have "+str(len(appointments))+" Appointments booked")
		for w, week in enumerate(self.cal.monthdayscalendar(y, m), 2):
			for d, day in enumerate(week):
				if day:
					...
					btn = Button(self.parent, text=day, bg=color, command=lambda day=day: self.selection(day, calendar.day_name[day % 7]))
